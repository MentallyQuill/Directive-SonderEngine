...
| UI preferences and campaign library | Directive install/story scope | The six-package library is bundled presentation data; People collection preferences persist per Directive campaign and Sonder story, never as campaign truth |
...
Sonder's committed turn replaces SillyTavern's accepted-pair boundary. A Sonder turn is provisional until commit. Once committed, its turn id and active lineage are the source binding. A reroll restores the pre-turn checkpoint and replays the beat; branches, deletion and rollback carry or remove Directive frame state through Sonder's native namespaced storage. Directive never implements a second accepted-message timeline. Command Bearing reservations snapshot the latest host turn, remain inert during the request that created them, and arm only when `on_director_payload` observes the next real Sonder player-turn id; settlement can commit that reservation only to the same turn.
...
Authored packages name actors before Sonder assigns numeric ids. Each participant therefore carries a private, versioned `directive.packageActorBinding.v1` containing the package id/version and portable `actor_ref`. The binding is resolved through Sonder's character handle and never exposed in the player DTO. Its authored package version is preserved so older stories remain readable after a package update. Existing exact `directive.crewDomain.v1` values migrate record-by-record to `directive.crewProfile.v2`; malformed or unknown profile versions are left untouched rather than guessed.
...
All gameplay data is fetched from Directive routes that assemble player-safe DTOs. Presentation state never writes campaign truth. The Playwright gate covers onboarding and all five routes at 1440×900, 1024×768, 390×844, 360×800, and 360×500, plus both sides of every discovered responsive width across all routes. It verifies exact route order and active identity, LCARS tokens, responsive rail/shelf geometry, controls, decoded media, internal scroll ownership, keyboard focus entry/restoration, Escape close, roving route focus, modal focus/inert/busy behavior, normal and reduced motion, computed type stacks, Chromium platform fonts, and browser/request failures. Full-resolution shell clips emit actual, authoritative reference, and per-pixel difference images by pausing production CSS animations at their authored phase zero. Settings remains structurally and stylistically exact while only measured content-specific text/fact regions are excluded from raster comparison; the cards and every remaining layout/style contract are asserted directly at all five viewports. Focus restoration resolves the current host launcher by its stable `data-ext-button` id because Sonder may rebuild the toolbar while the view is open. The Sonder bridge gives the launcher, route glyphs, reorder handle, and player-portrait control masks explicit extension-asset URLs because relative CSS URLs otherwise resolve against Sonder's aggregated stylesheet endpoint.

Campaign opens first on every launch. With a story it opens the animated layered Current Campaign dashboard; without one it opens the six-package campaign library. Ashes is playable and the other five packages expose the exact current Directive teaser art, copy, metadata, grayscale states, and disabled creation behavior. Save Game clones the current Sonder story and records the immutable clone in Directive metadata; Load Game imports the chosen clone as a new story and registers the story it replaced as the previous timeline; Delete unregisters metadata before deleting the clone and restores the registration if host deletion fails. Sonder remains the owner of all story data and lineage.

People collection/category order is a per-campaign, per-story presentation preference with pointer, touch, and keyboard reordering. Mobile Campaign and Mission records use one-open accessible disclosures. Ship callouts solve from package-authored image anchors, and a Ship Cohesion Command Bearing reservation uses the same real-turn custody described above. Campaign/timeline dialogs trap focus, make the underlying Directive shell inert, lock dismissal while async work is busy, and remain retryable after failure. Character creation uses Directive's four-step commissioning UI, keeps drafts session-ephemeral until atomic provisioning, and invokes its bounded assistant only on explicit player action through the dedicated `creator-assist` Sonder model lane. Persistent gameplay notices remain migration work. Settings intentionally states the Sonder ownership boundary instead of duplicating provider controls. On desktop, Sonder's story surface remains dimly visible behind Directive's modal overlay; on mobile the Directive console occupies the viewport. Those host-owned surfaces are not restyled by Directive.
