...
People collection/category order is a per-campaign, per-story presentation preference with pointer, touch, and keyboard reordering. Mobile Campaign and Mission records use one-open accessible disclosures. Ship callouts solve from package-authored image anchors, and a Ship Cohesion Command Bearing reservation uses the same real-turn custody described above. Campaign/timeline dialogs trap focus, make the underlying Directive shell inert, lock dismissal while async work is busy, and remain retryable after failure. Persistent gameplay notices and bounded creator assist remain migration work. Settings intentionally states the Sonder ownership boundary instead of duplicating provider controls. On desktop, Sonder's story surface remains dimly visible behind Directive's modal overlay; on mobile the Directive console occupies the viewport. Those host-owned surfaces are not restyled by Directive.
