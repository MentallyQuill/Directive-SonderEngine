...
That browser proof verifies the responsive host integration and the source presentation hierarchy for Campaign, Mission, People, Ship, Settings and onboarding; it is not a claim that every backend-powered interaction has migrated. Campaign save/load/delete and multi-package management, persistent gameplay notices, creator assist, people reordering/categories and Command Bearing actions remain in progress. Settings intentionally states the Sonder ownership boundary instead of duplicating provider controls. On desktop, Sonder's story surface remains dimly visible behind Directive's modal overlay; on mobile the Directive console occupies the viewport. Those host-owned surfaces are not restyled by Directive.
