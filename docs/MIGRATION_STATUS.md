...
| Route workspace visual parity | Verified | Live current-Sonder captures use the source Campaign dashboard, Mission master/detail, People roster/detail, Ship cohesion orbit/callouts, Settings sections, and exact responsive shell composition at 1440×900 and 390×844; unsupported mutations remain tracked separately |
...
- Side-by-side review used current Directive conformance renders, not the earlier simplified Sonder UI. The LCARS tokens, centered overlay, responsive shell, route hierarchy, Campaign hero, Mission journal, People roster/detail, and Ship cohesion workspace align. On desktop, Sonder's host-owned Stories surface remains dimly visible behind the Directive overlay; on mobile Directive occupies the viewport. Unsupported backend mutations remain explicitly separate from visual parity.
