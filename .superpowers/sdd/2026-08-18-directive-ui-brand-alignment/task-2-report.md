...
- Rendered the final source dashboard from literal projection values: campaign title, player identity, exact Ashes package ship identity, current mission id/title/summary, and the route-native ship-time chronometer. Library and Records retain their allowlisted projection metadata. Missing projection values remain explicit or omitted according to the source surface; no generic campaign title, player identity, mission, or location is invented.
